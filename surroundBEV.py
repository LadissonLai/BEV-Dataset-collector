...
class Camera:
    def __init__(self, name):
        ...
        self.camera_mat = np.load(os.path.dirname(abspath) + '/data/{}/camera_{}_K.npy'.format(name,name))
        self.dist_coeff = np.load(os.path.dirname(abspath) + '/data/{}/camera_{}_D.npy'.format(name,name))
        self.homography = np.load(os.path.dirname(abspath) + '/data/{}/camera_{}_H.npy'.format(name,name))
...
def main():
    front = cv2.imread('./data/front/front1.png')
    back = cv2.imread('./data/back/back1.png')
    left = cv2.imread('./data/left/left1.png')
    right = cv2.imread('./data/right/right1.png')
